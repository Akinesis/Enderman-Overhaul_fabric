...
public class SummonerPearlItem extends EnderpearlItem {
    public SummonerPearlItem(Properties properties) {
        super(properties);
    }
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level level, Player player, @NotNull InteractionHand usedHand) {
        ...
        if (!level.isClientSide()) {
            ThrownSummonerPearl pearl = new ThrownSummonerPearl(level, player);
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag isAdvanced) {
        tooltipComponents.add(ConstantComponents.SUMMONER_PEARL_TOOLTIP);
    }
}
