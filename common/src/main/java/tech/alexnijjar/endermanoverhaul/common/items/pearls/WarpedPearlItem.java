...
public class WarpedPearlItem extends EnderpearlItem {
    public WarpedPearlItem(Properties properties) {
        super(properties);
    }
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level level, Player player, @NotNull InteractionHand usedHand) {
        ...
        if (!level.isClientSide()) {
            ThrownWarpedPearl pearl = new ThrownWarpedPearl(level, player);
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag isAdvanced) {
        tooltipComponents.add(ConstantComponents.WARPED_PEARL_TOOLTIP);
    }
}
